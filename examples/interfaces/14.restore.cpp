 ...
 *	MODULE:		14.restore.cpp
 *	DESCRIPTION:	A sample of making restore of database using service manager.
 *
...
int main()
{
	...
	try
	{
		...
		printf("** Demo of making restore using service manager...\n");

		// Build service start SPB
		spb2 = utl->getXpbBuilder(&status, IXpbBuilder::SPB_START, NULL, 0);
		spb2->insertTag(&status, isc_action_svc_restore);
		spb2->insertString(&status, isc_spb_dbname, "employee-restored.fdb");
		spb2->insertString(&status, isc_spb_bkp_file, "employee.fbk");
		spb2->insertInt(&status, isc_spb_options, isc_spb_res_create);
