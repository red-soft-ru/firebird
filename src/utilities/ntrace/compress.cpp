...
THREAD_ENTRY_DECLARE compress_file(THREAD_ENTRY_PARAM arg)
{
	...
	Firebird::PathName src_filename(source_file);
	Firebird::PathName dst_filename(source_file);
	dst_filename += ".gz";
	delete[] source_file;

	Firebird::PathName path, filename;
	PathUtils::splitLastComponent(path, filename, src_filename);
	...
	try
	{
		...
		const int fd = os_utils::open(src_filename.c_str(),
#ifdef WIN_NT
		...
		if (fd < 0)
		{
			...
			gds__log("PluginLogWriter: operation \"open\" failed on file \"%s\". Error code: %d", src_filename.c_str(), errno);
			return 0;
		}
		...
		do
		{
			...
			else if (bytes_readed < 0)
			{
				close(fd);
				zlib().gzclose(gzip_file);
				gds__log("PluginLogWriter: operation \"read\" failed on file \"%s\". Error code: %d", src_filename.c_str(), errno);
				return 0;
			}
		...
		if (close(fd) < 0)
			gds__log("PluginLogWriter: operation \"close\" failed on file \"%s\". Error code: %d", src_filename.c_str(), errno);
	}
